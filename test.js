...
const VipNode = ({ data }) => {
  ...
  return (
    <div className={`px-6 py-4 rounded-xl border-2 text-center min-w-[180px] shadow-2xl transition-colors ${
      isDark 
...
const BadgeNode = ({ data }) => {
  ...
  return (
    <div className={`px-4 py-2.5 rounded-lg border text-xs font-mono shadow-md text-center min-w-[130px] transition-colors cursor-grab active:cursor-grabbing ${getBadgeStyle()}`}>
...
// 🎯 REACT FLOW NODE TÜRLERİ (Bileşen dışında sabit tanımlanmalıdır)
const nodeTypes = {
  vipNode: VipNode,
  badgeNode: BadgeNode,
};

// 🧮 ARTIRILMIŞ MESAFELİ KOORDİNAT HESAPLAYICI
  ...
  if (rawData.pool) {
    ...
    (rawData.monitors || []).forEach((m, idx) => {
      ...
      nodes.push({
        id: monId,
        type: 'badgeNode',
        position: { x: POOL_X + 280, y: POOL_Y + (idx * 65) },
...
export default function ServiceMap({ mode }) {
  ...
  // 🎯 Sürükleme takibi için bağımsız state hook'ları
  ...
  // 3. Veri veya Tema Değiştiğinde Düğümleri Yerleştir
  useEffect(() => {
    if (rawData) {
